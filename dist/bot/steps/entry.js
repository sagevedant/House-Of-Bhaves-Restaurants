...
async function handleEntry(event, conversation, restaurant, stepData) {
    ...
    if (event.type === 'button_reply') {
        ...
        if (event.buttonId === 'view_menu') {
            await (0, sender_1.sendText)(restaurant, phone, '📋 Here is our menu link: https://spicefactory.com/menu');
            return { nextStep: 'entry', stepData };
        }
        if (event.buttonId === 'talk_to_us') {
            await (0, sender_1.sendText)(restaurant, phone, `📞 You can reach our manager at ${restaurant.managerPhone || '+123456789'}.`);
            return { nextStep: 'entry', stepData };
        }
        ...
        if (event.buttonId === 'modify_booking') {
            await (0, sender_1.sendText)(restaurant, phone, 'Please call the restaurant to modify your booking.');
